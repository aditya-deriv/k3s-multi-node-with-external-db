...
curl -sfL https://get.k3s.io | INSTALL_K3S_EXEC="--tls-san 10.0.1.10" INSTALL_K3S_VERSION=v1.26.4+k3s1 sh -s - server --token=${k3s_master_nodes_token} --datastore-endpoint="http://${k3s_db_private_ip}:2379" --node-taint node-role.kubernetes.io/master:NoSchedule --write-kubeconfig-mode 644
